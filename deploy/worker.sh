...
# Todo lo que es especificación del otro equipo. Se pasa tal cual al contenedor: cambiar
# cualquiera de estas no obliga a reconstruir la imagen (ver Config.java).
TP_COLA_URLS="${TP_COLA_URLS:-}"
...
# Cómo se identifica cada worker ante la cola. NO es un nombre libre: el contrato pide el
# host:puerto gRPC de la réplica, porque es el string que el balanceador publica en su
# /health como "réplicas que están consumiendo". Uno inventado deja a la réplica figurando
# como sana y sin consumir nada.
#
# Se arma como <TP_COLA_HOST>:<PUERTO_BASE + n>, que da 8111, 8112, … — los mismos puertos
# que usan las réplicas gRPC del color azul (ver deploy/entorno.sh). Con un solo worker se
# puede fijar TP_COLA_CONSUMIDOR a mano y listo.
TP_COLA_HOST="${TP_COLA_HOST:-}"
PUERTO_BASE_GRPC="${PUERTO_BASE_GRPC:-8110}"
...
# Cómo se presenta el worker n ante la cola. Con TP_COLA_CONSUMIDOR fijo, todos usan el
# mismo string — sirve para un solo worker; con varios, cada uno tiene que ser distinto o la
# cola los cuenta como uno.
consumidor_de() {  # <n>
  if [[ -n "${TP_COLA_CONSUMIDOR:-}" ]]; then
    echo "$TP_COLA_CONSUMIDOR"
  else
    echo "$TP_COLA_HOST:$(( PUERTO_BASE_GRPC + $1 ))"
  fi
}
...
levantar_uno() {  # <n>
  ...
  MSYS_NO_PATHCONV=1 docker run -d --name "$nombre" --network "$RED" \
    ...
    -e TP_COLA_CONSUMIDOR="$(consumidor_de "$n")" \
  ...
  log "arriba $nombre · panel en :$puerto · como $(consumidor_de "$n")"
}
...
diagnostico() {
  ...
  if [[ -z "${TP_COLA_CONSUMIDOR:-}" && -z "$TP_COLA_HOST" ]]; then
    error "falta TP_COLA_HOST (o TP_COLA_CONSUMIDOR): es el host:puerto gRPC con el que"
    error "  el balanceador identifica a esta réplica, y sin eso figura como que no consume"
    algo_falla=1
  else
    ok "consumidor del worker 1: $(consumidor_de 1)"
  fi
...
levantar() {
  ...
  if [[ -z "$TP_COLA_URLS" ]]; then
    error "falta TP_COLA_URLS: es la URL del servicio de cola (GET para tomar, POST para devolver)"
    error "acepta varias réplicas separadas por coma"
  ...
  if [[ -z "${TP_COLA_CONSUMIDOR:-}" && -z "$TP_COLA_HOST" ]]; then
    error "falta TP_COLA_HOST: la IP de tailscale de esta casa"
    error "con eso cada worker se presenta como <ip>:$(( PUERTO_BASE_GRPC + 1 )), <ip>:$(( PUERTO_BASE_GRPC + 2 ))…,"
    error "que es el host:puerto gRPC con el que el balanceador identifica a la réplica."
    error "Un identificador inventado la deja figurando como sana y sin consumir nada."
    exit 2
  fi
  ...
  log "base: $(destino_redis) · casa: $CASA · workers: $cuantos × $TP_COLA_HILOS hilo(s)"
